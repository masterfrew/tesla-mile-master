...
serve(async (req: Request) => {
  try {
    ...
    if (req.method === "GET" && path.endsWith("/authorize")) {
      ...
      if (error) return new Response("state-store-failed", { status: 500 })
    ...
    if (req.method === "GET" && path.endsWith("/callback")) {
      ...
      const { data: st, error: stErr } = await supabase
        ...
        .eq("state", state)
        .single()
      if (stErr || !st) return new Response("invalid-state", { status: 400 })
      ...
      if (!r.ok) return new Response(`token-exchange-failed: ${await r.text()}`, { status: 500 })
      ...
      const { data: userInfo } = await sbAuth.auth.getUser()
      const userId = userInfo?.user?.id
      if (!userId) return new Response("no-signed-in-user", { status: 401 })

      const { error: upErr } = await supabase.from("tesla_tokens").upsert({
        user_id: userId,
      ...
      if (upErr) return new Response("token-store-failed", { status: 500 })
